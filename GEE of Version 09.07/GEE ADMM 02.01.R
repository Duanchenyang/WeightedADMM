#####################################################
##This is alpha^|j1-j2|*beta^indicator function######
#####################################################
...
cal_alpha_beta <- function(B_old,X,y,individual_time){
  ...
  for (subject in unique(individual_time$individual)){
    ...
    firstrow <- kronecker(matrix(capturenumber,nrow=1),matrix(rep(1,K),nrow=1))
    j1minusj2 <- rep.row(firstrow,length(firstrow))-t(rep.row(firstrow,length(firstrow)))
    ##change beta###
    indicatorbeta <-kronecker(ones(numtime,numtime),diag(rep(1,K))) 
    ...
    indicator_beta <- as.vector(indicatorbeta)[Rijkl!=0]
    fordata<- data.frame(newRijkl ,abs_j1_j2,indicator_beta) 
    regressiondata<- rbind(regressiondata, fordata)
  }
  
  ###change model#####
  #print(sd(regressiondata$newRijkl))
  mod <-summary(nls(newRijkl~(Alpha^abs_j1_j2)*((B2)^indicator_beta),data=regressiondata,start = list(Alpha = 0.2, B2 = 1),algorithm="port" ))
  ...
  cor_Beta <- mod$coefficients[2,1]
  return( list(cor_Alpha, cor_Beta))
...
Grad_L<- function(B_old,X,y,individual_time){
  ...
  for(subject in unique(individual_time$individual)){
    ...
    capturenumber <- subset(individual_time,individual==subject)$Capture.Number
    ####might need change#########
    corR <-kronecker(toeplitz(c(0,cor_Alpha^(capturenumber[-1]-1))),ones(K,K)-diag(rep(1,K))+diag(cor_beta,K))
